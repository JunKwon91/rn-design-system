...
## ADR-38: Dark 팔레트 재설계 — 예약 (미착수)

### 상황

ADR-37 토큰 정합화 과정에서 확인된 구조적 한계의 의사결정 이력 기록.

**Dark surface 5단의 좁은 명도 영역**:

| 단계 | hex | Y | L* |
|------|------|------|------|
| containerLowest | `#0B0E15` | 0.00438 | 6 |
| containerLow | `#191B23` | 0.01113 | 10 |
| container | `#1D2027` | 0.01443 | 12 |
| containerHigh | `#272A31` | 0.02310 | 17 |
| containerHighest | `#2F333B` | 0.03290 | 21 |

L* 범위가 6~21로 압축되어 있다. 이 영역 안에 `border.subtle`이 어떤 값으로 들어가든 인접 surface(특히 container ↔ containerHigh)와 ΔL* 1~3으로 좁아지는 한계가 있다. ADR-37에서 `slateDark-870 #1F2229 (L*≈13)`로 분리해 동일 hex 충돌은 해소했으나, container(L*=12) 위에서는 ΔL*≈1로 가시성이 매우 약하다.

이는 **토큰값으로는 해소 불가** — Dark surface 계층 자체의 명도 범위를 더 넓게 가져가야 구조적으로 해결된다.

### 선택

**Dark 팔레트 재설계는 별도 사이클로 분리. 본 ADR은 예약(미착수).**

- 작업 영역: Dark surface 5단의 명도 범위 확장 (예: L* 4~30 또는 그 이상), slateDark 계열 primitive 재배치, Dark 화면 전체 인상 변경 동반.
- 영향 영역: 갤러리 9화면 Dark 모드 전수 회귀, 모든 Dark 컴포넌트 시각 인상 변화.
- 본 ADR은 의사결정 이력 기록용. 착수 시점·범위는 별도 결정.

### 포기한 옵션

| 옵션 | 사유 |
|------|------|
| ADR-37 라운드에서 Dark 팔레트도 함께 재설계 | 영향 영역(모든 Dark 화면)이 토큰 정합화 범위를 크게 초과. 분리가 정직 |
| border.subtle 가시성을 사용 가이드만으로 회피 | container 위 ΔL*≈1은 가이드로 회피 어려움. 구조적 해결이 우선 |
| `surface.dim` 부활시켜 Dark에서 추가 단계 확보 | dim은 ADR-37에서 사용 0으로 삭제 — 의미 모호 토큰 부활은 정의-사용 정합 깨짐 |

### 근거

- Dark surface 명도 범위는 OLED 기준·콘트라스트 인지·페이지 인상 모두에 걸친 결정 영역. 토큰 정합화 사이클과는 영향 영역 단위가 다르다.
- 본 ADR은 "착수 안 함"의 명시적 기록. 후속 라운드에서 본 ADR을 근거로 작업 영역을 정의한다.

### 결과

- 상태: **예약 (미착수)**. 본 ADR 작성 외 코드 변경 0.
- 후속 착수 시 본 ADR을 supersede / 확장하는 별도 ADR로 결정 기록.
