...
Card 컴포넌트의 Variant가 `{Default(테두리), Elevated(그림자)}` × Density였다. 그러나 사용자 앱(LottoStats)에서 카드는 "독립 정보 패널"과 "반복 리스트 항목" 두 용도로 쓰이는데 기존 variant가 이 구분과 맞지 않았다 — 그림자형(Elevated)은 실사용이 0이었고, 앱은 "테두리 있는 카드 / 없는 카드" 2종을 필요로 했다.
...
- **사용처 매핑(리스트=Filled, 패널=Outlined)은 DS가 아니라 사용자 앱이 정의한다** — DS가 특정 앱의 의미론을 알면 재사용성이 깨지므로. (LottoStats ADR-10)
...
사용자 앱(LottoStats)에 "옵션 하나를 고르는 선택형 카드"(예: 번호 추천 알고리즘 선택)가 있는데, 선택 표시가 컴포넌트가 아니라 임시 요소로 제각각이었다 — 선택=Check 인스턴스 / 미선택=직접 그린 원. 이를 DS 컴포넌트로 정식화하려 했으나 두 벽에 부딪혔다:
...
| OptionCard에 아이콘 내장 | 알고리즘 아이콘은 앱 도메인 → 아이콘 슬롯(instance-swap)으로 두고 사용자 앱이 제공 |
...
- 사용자 앱은 OptionCard 인스턴스에 자기 아이콘(예: AlgorithmIcon)을 슬롯 swap + 로컬 테마 색으로 사용(LottoStats 번호 추천 알고리즘 카드에 적용).
