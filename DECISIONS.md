...
- v2.0.0에서 npm 패키지로 분리한 뒤, 소비자 앱이 자기 theme을 styled `DefaultTheme`에 확장하려고 augmentation을 추가하면 라이브러리의 augmentation과 전역 단일 슬롯에서 충돌한다. `declare module 'styled-components'`는 전역 declaration merging이라 라이브러리가 이미 점유한 모양이 소비자 확장을 가려 버린다.
...
- 소비자 앱은 `import type { AppTheme } from '@junkwon91/rn-design-system'`로 라이브러리 타입을 받아, 자기 augmentation을 `declare module 'styled-components' { ... }`로 자유롭게 작성.

### 포기한 옵션

| 옵션 | 사유 |
|------|------|
| 라이브러리 augmentation 유지 | 소비자 확장 충돌 — v2.0.0 분리 목적 자체와 정면 충돌 |
...
- **책임 경계**: 라이브러리는 "theme의 모양"(타입)은 정의하되 "전역 슬롯 점유"는 소비자 결정 영역으로 둔다 — declaration merging 단일 슬롯의 본질 제약을 책임 분리로 회피.
- **2층 자동완성 보강**: useAppTheme 헬퍼는 styled 외 본문(컴포넌트 본문 콜백, 비-styled 영역)에서 풀 자동완성. styled 콜백은 인자 명시로 콜백 본문 자동완성·오타 검사 확보.
- **소비자 API 영향 0건**: 라이브러리가 노출하는 컴포넌트·훅·타입은 추가만 있고 제거·시그니처 변경 0.
...
- 소비자는 styled `DefaultTheme`을 자유롭게 augmentation 가능 — 라이브러리 점유 0
...
| 라이브러리 augmentation 부활 | ADR-39 결정과 정면 충돌 — 소비자 확장 차단으로 v2.0.0 분리 의도 무효화 |
...
- **출하 .d.ts 정확성**: `noCheck`은 emit 경로의 타입체크만 스킵 — 결과 .d.ts에는 타입 정보가 그대로 들어감. 소비자 API에는 영향 없음(콜백 인자 좁힘은 styled 내부 구조 영향, 라이브러리 export 타입에는 영향 없음).
...
Card 컴포넌트의 Variant가 `{Default(테두리), Elevated(그림자)}` × Density였다. 그러나 소비 앱(LottoStats)에서 카드는 "독립 정보 패널"과 "반복 리스트 항목" 두 용도로 쓰이는데 기존 variant가 이 구분과 맞지 않았다 — 그림자형(Elevated)은 실사용이 0이었고, 앱은 "테두리 있는 카드 / 없는 카드" 2종을 필요로 했다.
...
| 단일 스타일(variant 없음) | 소비 앱의 패널/리스트 구분을 표현 불가 |
| 콘텐츠 슬롯(임의 자식 주입) | Figma는 퍼블리시 인스턴스에 임의 자식을 중첩할 수 없음 — 근본적으로 불가 |

### 근거

- 테두리 유무에 "패널 vs 리스트"라는 의미를 담아, 소비 앱이 variant를 고민 없이 선택할 수 있게 한다.
